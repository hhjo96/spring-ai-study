...
public class BookService {
    ...
    public Book registerBook(BookCreateRequest request) {
        ...
        // 2) RDB에 저장
        Book saved = bookRepository.save(book);
        log.info("도서 RDB 저장 완료: [{}] {} (ID: {})", saved.getGenre(), saved.getTitle(), saved.getId());

        // 3) VectorStore에 동기화 (임베딩 + 저장)
        Document document = toDocument(saved);
        vectorStore.add(List.of(document));
        log.info("도서 VectorStore 동기화 완료: [{}] {} → 시맨틱 검색 가능", saved.getGenre(), saved.getTitle());
